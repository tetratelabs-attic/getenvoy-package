...
def check_cloudsmith(args: Any) -> int:
    ...
    try:
        ...
        logging.error("Failed to check if file exists in Cloudsmith: %s",
                      err.output)
        return 1
    ...
    try:
        ...
        logging.debug("the file %s does not already exist in the repo",
                      args.filename)
...
def upload_to_cloudsmith_raw(args, override=False) -> None:
    ...
    try:
        ...
        logging.error("Failed to upload file %s to cloudsmith: %s",
                      args.filename, err.output)
        raise
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=False,
        help=  # noqa: E251
        "Whether to override the already existing file with the same name and version",
    )
    parser.add_argument(
        "--cloudsmith_auth",
    ...
    parser.add_argument(
        ...
        help=  # noqa: E251
        "A base string to generate the actual DEB repos with e.g. stable/nightly suffixes",
    )
    parser.add_argument(
        "--cloudsmith_repo_rpm_base",
        default=os.environ.get("CLOUDSMITH_REPO_RPM_BASE", "getenvoy-rpm"),
        help=  # noqa: E251
        "A base string to generate the actual RPM repos with e.g. stable/nightly suffixes",
    )
    parser.add_argument(
        "--release_level",
